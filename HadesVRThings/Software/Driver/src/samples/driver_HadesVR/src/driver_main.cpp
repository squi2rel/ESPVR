...
EVRInitError CServerDriver_Sample::Init( vr::IVRDriverContext *pDriverContext )
{
	...
	else if (strcmp(transportMode, "SOCKET") == 0 || strcmp(transportMode, "Socket") == 0 || strcmp(transportMode, "socket") == 0) {
		DriverLog("[Init] Using Socket transport");
		dataTransport = new SocketTransport();
	}
	else {
	...
	{
		DriverLog("[Init] HeadsetConnected");
		if (HMDEnabled) {
			HMDConnected = true;
		}
		if (ctrlsEnabled) {
	...
	{
		DriverLog("[Init] HeadsetNOTConnected");
		HMDConnected = false;
